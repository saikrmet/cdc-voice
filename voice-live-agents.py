...
# Track when TTS audio is being received/played to avoid echo-triggered VAD and to pause mic
...
class AudioPlayerAsync:
    ...
    # New: helpers to know when playback has fully drained
    def buffered_frames(self) -> int:
        with self.lock:
            return sum(len(chunk) for chunk in self.queue)

    def wait_drain(self, timeout: float = 10.0):
        start = time.time()
        while self.buffered_frames() > 0 and (time.time() - start) < timeout:
            time.sleep(0.01)
    ...
    try:
        ...
        while not stop_event.is_set():
            if stream.read_available >= read_size:
                ...
                # If TTS is active, drop mic frames to avoid echo confusing VAD
                if tts_active.is_set():
                    continue

                # Encode raw PCM bytes, not the NumPy array object
    ...
    try:
        while not stop_event.is_set():
            ...
            try:
                ...
                # Reduce console I/O during streaming to avoid starving the audio callback
                ...
                if event_type == "session.created":
                    ...
                    # A new agent response is starting; mark TTS active to pause mic echo
                    tts_active.set()

                elif event_type == "response.audio_transcript.done":
                    # Defer heavy printing until audio buffer has drained
                    pending_agent_transcript = event.get("transcript", "")
                    audio_player.wait_drain(timeout=10.0)
                    if pending_agent_transcript is not None:
                        agent_audio = f'Agent Audio Response:\t{pending_agent_transcript}'
                        print(f'\n\t{agent_audio}\n')
                    ...
                    # Mark TTS active while audio chunks are coming in
                    tts_active.set()
                    ...
                    # Ensure any remaining buffered audio is rendered
                    audio_player.wait_drain(timeout=10.0)
                    # Clear TTS active to resume mic capture
                    tts_active.clear()

                elif event_type == "input_audio_buffer.speech_started":
                    # Ignore echo-triggered VAD during TTS so we don't kill playback
                    print("Speech started (ignored)")
                    # Do NOT call audio_player.stop()

                elif event_type == "input_audio_buffer.speech_stopped":
                    # Explicitly commit the turn so the server processes the user utterance
                    try:
